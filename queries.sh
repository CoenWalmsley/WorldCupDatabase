...
echo "$($PSQL "SELECT AVG(winner_goals) AS avg_goals_winning_teams FROM games")"

echo -e "\nAverage number of goals in all games from the winning teams rounded to two decimal places:"
echo "$($PSQL"SELECT ROUND(AVG(winner_goals), 2) AS avg_goals_winning_teams_rounded FROM games")"

echo -e "\nAverage number of goals in all games from both teams:"
echo "$($PSQL"SELECT AVG(winner_goals + opponent_goals) AS avg_goals_all_teams FROM games")"

echo -e "\nMost goals scored in a single game by one team:"
echo "$($PSQL"SELECT GREATEST(MAX(winner_goals), MAX(opponent_goals)) AS most_goals_in_game FROM games")"
...
echo "$($PSQL"SELECT g.year, t.name FROM games g JOIN teams t ON g.winner_id = t.team_id WHERE round='Final' ORDER BY year")"
